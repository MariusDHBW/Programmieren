...
function setupCharacter(a, b)
    ...
    for k, v in pairs(starters) do
        ...
        local options = {
            rotation = {0, 180, 0},
            position = {a.getPosition().x + 2 + card_offset, a.getPosition().y + 1, a.getPosition().z}
        }
        if special ~= nil then
...
function setupCharacterF(a,b)
  ...
  local hp = getObjectFromGUID(hp_bag).takeObject({
    position = {a.getPosition().x - 8, a.getPosition().y + 2, a.getPosition().z + 6}
  })
  local san = getObjectFromGUID(san_bag).takeObject({
    position = {a.getPosition().x - 8, a.getPosition().y + 2, a.getPosition().z + 4}
  })
  ...
  if tokenGMNotes == "special" then
	...
	tokenX = tonumber(string.sub(tokenGMNotes, 1,5))
	tokenZ = tonumber(string.sub(tokenGMNotes, -5))
  ...
  if shouldGetPersonalStory == "True" then
	...
	story.setPosition(vector(a.getPosition().x+3,a.getPosition().y+2,a.getPosition().z+3))
	local storyEnd = searchForInExact(a.getData()["Nickname"],personalStories_deck)
	storyEnd.setPosition(vector(a.getPosition().x+5,a.getPosition().y+2,a.getPosition().z+3))
  end
end
...
function setupCharacterW(a,b)
  ...
  local hp = getObjectFromGUID(hp_bag).takeObject({position={a.getPosition().x-8,a.getPosition().y+2,a.getPosition().z+6}})
  local san = getObjectFromGUID(san_bag).takeObject({position={a.getPosition().x-8,a.getPosition().y+2,a.getPosition().z+4}})
  hp.Counter.setValue(a.getVar('hp'))
  san.Counter.setValue(a.getVar('san'))
  token.setColorTint(stringColorToRGB(b))
  local tokenData = token.getData()
